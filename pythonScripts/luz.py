...
def main(output_file, output_neg_file, selector_url, estado_url, apagado_url):
    ...
    try:
        ...
        # Apagar todas las luces y desexportar los pines
        for pin in output_pins.values():
            set_pin_value(pin, "0")
        for pin in output_neg_pins.values():
            set_pin_value(pin, "1")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Script para manejar las luces automáticamente.')
    parser.add_argument('output_file', type=str, help='Archivo de configuración de luces (lógica positiva).')
    parser.add_argument('output_neg_file', type=str, help='Archivo de configuración de luces (lógica negativa).')
