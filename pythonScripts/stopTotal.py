...
def turn_on_pins(pins_file, neg_pins_file):
    # Leer los pines desde los archivos
    with open(pins_file, "r") as f:
        pins = [int(line.strip()) for line in f if line.strip().isdigit()]
        
    with open(neg_pins_file, "r") as f:
        neg_pins = [int(line.strip()) for line in f if line.strip().isdigit()]
    ...
    for pin in pins:
        try:
            ...
            if pin in neg_pins:
                print(f"Turning on GPIO pin {pin} with negative logic...")
                set_pin_value(pin, 0)  # Lógica negada
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('neg_pins_file', type=str, help='El archivo con la lista de pines GPIO con lógica negada.')

    args = parser.parse_args()
    turn_on_pins(args.pins_file, args.neg_pins_file)
