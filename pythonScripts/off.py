...
def turn_off_pins(pins_file):
    ...
    for pin in pins:
        try:
            ...
            print(f"Turning off GPIO pin {pin}...")
            set_pin_value(pin, 0)
            unexport_pin(pin)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Script para apagar todos los pines listados en un archivo.')
    parser.add_argument('pins_file', type=str, help='El archivo con la lista de pines GPIO.')

    args = parser.parse_args()
    turn_off_pins(args.pins_file)
